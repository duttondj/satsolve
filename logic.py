...
def WalkSAT(clauses, p=0.5, max_flips=10000):
    ''' This function will perform a WalkSAT algorithm to determine the best
        set of conditions for the clauses. It has been modified from the 
        original WalkSAT code from the Author's site. '''
    ## model is a random assignment of true/false to the symbols in clauses
    
    # Get a list of all the literals used in the clauses
    proplist = []
    for clause in clauses:
        proplist.extend(prop_symbols(clause))
    # The original code below does not work. WalkSAT expects a list of 
    # clauses but prop_symbols() expects a single clause. The uncommented
    # code following does work!
    # model = dict([(s, random.choice([True, False]))
    #              for s in prop_symbols(clauses)])
    
    # Build a model of random initial values for the literals
    model = dict([(s, random.choice([True, False]))
                 for s in proplist])
    # Keep the best number of satisfied clauses in case we run out of flips
    bestmaxsat = -1
    ...
    for i in range(max_flips):
        ...
        if probability(p):
            ...
            numsatlist = []
            litlist = []
            tempproplist = prop_symbols(clause)
            
            # Go through each literal
            for lit in tempproplist:
                model[lit] = not model[lit]     # flip it
                for clause in clauses:          # check every clause
                    if pl_true(clause, model):  # If its sat, increment the counter for that literal
                        numsat += 1
                model[lit] = not model[lit]     # reset model
                numsatlist.append(numsat)       # Create a list of the number of sat outcomes
                litlist.append(lit)             # Create a matching list of the literal
                numsat = 0
            
            maxsatnum = -1
            for n,i in enumerate(numsatlist):   # find the max sat lit
                if i >= maxsatnum:
                    maxsatnum = i
                    maxsatindex = n
            sym = litlist[maxsatindex]          # Use the literal with the higher number of sat

        model[sym] = not model[sym]

    # Max number of flips exceeded
    return (bestmodel, bestflips, 1)
